...
export default {
    ...
    <div class="fade-in">
        ...
        <div v-if="activeTab === 'Overview'" class="space-y-6">
            ...
            <div v-if="pendingTransactions.length > 0">
                <h3 class="text-base font-bold text-gray-700 pl-2 border-l-4 border-yellow-500 mb-4">Pending Transactions</h3>
                <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                    <table class="custom-table w-full text-left border-collapse">
                        <thead>
                            <tr>
                                <th class="p-3 bg-gray-50 text-xs font-medium text-gray-500 border-b">ID</th>
                                <th class="p-3 bg-gray-50 text-xs font-medium text-gray-500 border-b">Type</th>
                                <th class="p-3 bg-gray-50 text-xs font-medium text-gray-500 border-b">Product</th>
                                <th class="p-3 bg-gray-50 text-xs font-medium text-gray-500 border-b">Amount</th>
                                <th class="p-3 bg-gray-50 text-xs font-medium text-gray-500 border-b">Status</th>
                                <th class="p-3 bg-gray-50 text-xs font-medium text-gray-500 border-b">Date</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="txn in pendingTransactions" :key="txn.id" class="hover:bg-gray-50">
                                <td class="p-3 border-b text-xs font-mono text-gray-600">{{ txn.id }}</td>
                                <td class="p-3 border-b text-xs text-gray-600">{{ txn.type }}</td>
                                <td class="p-3 border-b text-xs text-gray-600">{{ txn.product }}</td>
                                <td class="p-3 border-b text-sm font-medium text-gray-800">
                                    {{ txn.type === 'Deposit' ? '+' : '-' }}₦{{ formatNumber(txn.amount) }}
                                </td>
                                <td class="p-3 border-b">
                                    <span class="px-2 py-0.5 rounded text-xs bg-yellow-50 text-yellow-600">
                                        <i class="fa-solid fa-clock mr-1"></i> {{ txn.status }}
                                    </span>
                                </td>
                                <td class="p-3 border-b text-xs text-gray-500">{{ txn.date }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div v-if="activePortfolioList.length" class="space-y-4">
                    <div v-for="item in activePortfolioList" :key="item.id" 
                        ...
                        <div class="p-4 grid grid-cols-4 gap-4 text-sm">
                            ...
                            <div><div class="text-xs text-gray-400 mb-1">Interest (₦)</div><div class="font-bold text-gray-800">{{ formatNumber(item.interest) }}</div></div>
            ...
            <div v-if="!branchName">
                ...
                <div class="flex items-center gap-4 mb-4">
                    <h3 class="text-base font-bold text-gray-700 pl-2 border-l-4 border-blue-500">Fixed Plans</h3>
                    
                    <!-- Category Filter Pills -->
                    <div class="flex bg-gray-100 p-1 rounded-full space-x-1">
                        <button v-for="cat in planCategories" :key="cat.id"
                                @click="selectedPlanCategory = cat.id"
                                class="px-4 py-1.5 text-xs font-bold rounded-full transition-colors"
                                :class="selectedPlanCategory === cat.id ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'">
                            {{ cat.name }}
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-4 gap-4">
                    <div v-for="plan in filteredStandardProducts" :key="plan.id" 
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-50 flex justify-between items-center text-xs">
                            <span class="text-gray-400">Min ₦{{ formatNumber(plan.minAmount) }}</span>
                            <span class="text-opay font-medium group-hover:underline">Create Plan</span>
                        </div>
                    </div>
                </div>
                
                <!-- Fallback empty state for categories if needed -->
                <div v-if="filteredStandardProducts.length === 0" class="text-center py-10 bg-white rounded-lg border border-dashed border-gray-200 text-gray-400 text-sm">
            ...
            <div v-if="visibleSpecialProducts.length" class="grid grid-cols-3 gap-6">
                <div v-for="product in visibleSpecialProducts" :key="product.id" 
                    ...
                    <div class="p-5">
                        ...
                        <!-- Quota Bar (Only show if totalSize is not -1) -->
                        <div v-if="product.totalSize !== -1" class="mb-4">
                        ...
                        <div v-else class="mb-4 text-xs text-gray-500 flex items-center justify-between">
                            <span class="flex items-center text-green-600 font-medium bg-green-50 px-2 py-1 rounded">
                                <i class="fa-solid fa-infinity mr-1.5 text-[10px]"></i> Unlimited Quota
                            </span>
                        </div>
        ...
        <div v-if="showBuyModal" class="modal-mask" @click.self="closeBuyModal">
            <div class="modal-content overflow-hidden">
                <div class="bg-opay p-4 text-white relative">
                    ...
                    <div class="text-xs opacity-80 mt-1" v-if="selectedProduct">
                        <span>Min: ₦{{ formatNumber(selectedProduct.minAmount) }} - Max: {{ selectedProduct.maxUserLimit === -1 ? 'Unlimited' : '₦' + formatNumber(calculateUserRemainingQuota(selectedProduct)) }}</span>
                    </div>
                </div>

                <div v-if="selectedProduct && selectedProduct.maxUserLimit !== -1 && calculateUserRemainingQuota(selectedProduct) < selectedProduct.minAmount" class="p-8 text-center">
    ...
    setup(props) {
        ...
        const planCategories = [
            { id: '7-60', name: '7-60 Days', min: 7, max: 60 },
            { id: '61-180', name: '61-180 Days', min: 61, max: 180 },
            { id: '181-365', name: '181-365 Days', min: 181, max: 365 },
            { id: '366-1000', name: '366-1000 Days', min: 366, max: 1000 }
        ];
        const selectedPlanCategory = ref('7-60');

        const filteredStandardProducts = computed(() => {
            const cat = planCategories.find(c => c.id === selectedPlanCategory.value);
            if (!cat) return standardProducts;
            return standardProducts.filter(p => p.days >= cat.min && p.days <= cat.max);
        });

        const pendingTransactions = ref([
            { id: 'PTXN8839202', type: 'Deposit', product: 'Standard 7 Days', amount: 50000, status: 'Pending', date: formatDateTime(new Date(props.currentTime.getTime() - 15 * 60000)) },
            { id: 'PTXN8839203', type: 'Withdraw', product: 'Fixed 27/01/2026', amount: 10000, status: 'Pending', date: formatDateTime(new Date(props.currentTime.getTime() - 45 * 60000)) }
        ]);
        ...
        const calculateUserRemainingQuota = (prod) => {
            if (prod.maxUserLimit === -1) return -1;
            return Math.max(0, prod.maxUserLimit - (prod.currentUserInvested || 0));
        };
        ...
        const validateInput = () => {
            ...
            if (investmentAmount.value < min) {
                inputError.value = `Minimum investment amount is ₦${formatNumber(min)}.`;
            } else if (max !== -1 && investmentAmount.value > max) {
                inputError.value = `Maximum investment amount is ₦${formatNumber(max)}.`;
            }
        };
        ...
        return {
            activeTab, standardProducts, specialProducts, visibleSpecialProducts, portfolioList, activePortfolioList, historyList, dashboardStats, pendingTransactions,
            ...
            tabs,
            planCategories, selectedPlanCategory, filteredStandardProducts
        };
    }
}